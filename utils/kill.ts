export const kill = (author: string, target: string) => {
	const killText = [
		...
		`${target} \\*\\*\\*\\* in eye, goes blind, runs for help but ran straight onto train tracks and gets plowed by a train.`,
	...
	return killText[Math.floor(Math.random() * killText.length)];
};
