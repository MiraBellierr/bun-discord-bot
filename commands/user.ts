import { SlashCommand, ApplicationCommandType, SlashCreator, CommandContext } from 'slash-create';
...
export default class AvatarCommand extends SlashCommand {
  ...
  async run(ctx: CommandContext) {
    ...
    const flags = target?.user.flags.toArray().map((flag) => flag.toLowerCase())
    .map((flag) =>
        flag
            .split("_")
    ...
    const embed = {
      ...
      description: `**• Username:** ${target?.user.username}\n**• Mention:** ${target?.user.mention}\n**• ID:** ${target?.user.id}\n**• Flags:** ${flags}\n**• Discriminator:** ${target?.user.discriminator}\n**• Type:** ${target?.user.bot ? "Bot" : "User"}\n**• Avatar URL:** [click here](${target?.avatarURL})\n**• Joined At:** ${joinedAt} (<t:${Math.round(target?.joinedAt ?? Date.now() / 1000)}:R>)`,
      color: 0xCD1C6C,
      image: { 
